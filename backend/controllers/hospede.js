...
controller.listar = async (req, res) => {
   ...
   if(Object.keys(req.query).length > 0) { // Se houver query string
      ...
      try {
         ...
         const lista = await Hospede.find()
            .populate({path:'reserva'})
            .populate({path:'cliente'})
            .populate({path:'quarto'})
...
controller.obterUm = async (req, res) => {
   ...
   try {
      ...
      const obj = await Hospede.findById(id)
...
controller.atualizar = async (req, res) => {
   try {
      ...
      const obj = await Hospede.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
   try {
      const id = req.body._id
      const obj = await Hospede.findByIdAndDelete(id)
      if (obj) {
         res.status(204).end()
      }
      else {
...
async function busca(req, res) {
   ...
   try {
      const lista = await Hospede.find(criterio)
      res.send(lista)
   }
   catch(erro) {
      console.log(erro)
...
controller.filtrarReserva = async (req, res) => {
   ...
   try {
      ...
      const lista = await Hospede.find({reserva: id})
         .populate('reserva')
         .populate('cliente')
         .populate('quarto')
      res.send(lista)
