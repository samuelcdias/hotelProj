...
controller.obterUm = async (req, res) => {
   ...
   try {
      ...
      const obj = await Funcionario.findById(id)
...
controller.atualizar = async (req, res) => {
   try {
      ...
      const obj = await Funcionario.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
   try {
      const id = req.body._id
      const obj = await Funcionario.findByIdAndDelete(id)
      if (obj) {
         res.status(204).end()
      }
      else {
...
async function busca(req, res) {
   ...
   try {
      const lista = await Funcionario.find(criterio)
      res.send(lista)
   }
   catch(erro) {
      console.log(erro)
