...
controller.obterUm = async (req, res) => {
   ...
   try {
      ...
      const obj = await Quarto.findById(id)
...
controller.atualizar = async (req, res) => {
   try {
      ...
      const obj = await Quarto.findByIdAndUpdate(id, req.body)
...
controller.excluir = async (req, res) => {
   try {
      const id = req.body._id
      const obj = await Quarto.findByIdAndDelete(id)
      if (obj) {
         res.status(204).end()
      }
      else {
...
async function busca(req, res) {
   ...
   try {
      const lista = await Quarto.find(criterio)
      res.send(lista)
   }
   catch(erro) {
      console.log(erro)
